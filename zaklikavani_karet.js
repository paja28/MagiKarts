    ...
    if(hrac_id_chybejicich_karet.length >0|| hrac_nebo_protihrac=="protihrac"){
        ...
        if(hrac_nebo_protihrac=="hrac"){
        nova_karta_img.classList.add("karty", "vysouvani_karet");
    ...
    function utok(karta_utocna){
        ...
        console.log(hrac_prostredek_objekty_karty);
        for(let i = 0;i<hrac_prostredek_objekty_karty.length;i++){
                if(hrac_prostredek_objekty_karty[i].id==karta_utocna.id){
                    utocna_karta_objekt = hrac_prostredek_objekty_karty[i];      //předělání id u objektu při přesunu
                    break;
                }
            }
            console.log(utocna_karta_objekt);
        for(let i =0; i < protihrac_utocne_karty.length;i++){
            document.getElementById(protihrac_utocne_karty[i]).setAttribute("onclick","snizeni_hp(\""+protihrac_utocne_karty[i]+"\", "+utocna_karta_objekt.id+")");
        }
    }
    ...
    function snizeni_hp(karta_snizeni_hp_id,utocna_karta_element){
        ...
        for(let i =0;i<protihrac_prostredek_objekty_karty.length;i++){  //nalezení hráčového objektu
            if(protihrac_prostredek_objekty_karty[i].id=karta_snizeni_hp_id)
            {
                index_poskozene_karty = i;
            }
        }
        for(let i =0;i<hrac_prostredek_objekty_karty.length;i++){  //nalezení hráčového objektu
            if(hrac_prostredek_objekty_karty[i].id=utocna_karta_element.id)
            {
                index_utocne_karty = i;
            }
        }
        protihrac_prostredek_objekty_karty[index_poskozene_karty].hp -= hrac_prostredek_objekty_karty[index_utocne_karty].dmg;
